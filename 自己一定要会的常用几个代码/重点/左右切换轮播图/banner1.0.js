/**
 * Created by Administrator on 2016/7/6 0006.
 */
//1.获取数据2.解析数据  3.将数据绑定在页面上  4.延迟加载 5.图片自动轮播 6.焦点自动轮播  7. 移入移出 8.点击焦点手动切换 9.点击左右按钮切换


//1.获取数据
(function(){
    var oBox=document.getElementById('box')
    var oBoxInner=oBox.getElementsByTagName('div')[0];
    var oUl=oBox.getElementsByTagName('ul')[0]
    var aDiv=oBoxInner.getElementsByTagName('div')
    var aImg=oBoxInner.getElementsByTagName('img1');
    var aLi=oBox.getElementsByTagName('li');
    var aA=oBox.getElementsByTagName('a');
    var data=null;
    var step=0;
    var autoTimer=null;
    var interval =1000;
//2.解析数据
  getData();
    function getData(){
        var xml=new XMLHttpRequest();
        //每一次加载出现避免缓存  -->?='+Math.random()
        xml.open('get','json/data.txt?='+Math.random(),false)
        xml.onreadystatechange=function(){
            if(xml.readyState===4&&/^2\d{2}$/.test(xml.status)){
               data= utils.jsonParse(xml.responseText)
            }
        }
        xml.send(null);
    }
    //console.log(data)
// 3.将数据绑定在页面上
    bind();
    function bind(){
        var str1='';//根据数据多少，进行字符串拼接div
        var str2='';//根据数据多少，进行字符串拼接li
        for(var i=0;i<data.length;i++){
        str1+='<div><img1 realImg="'+data[i].imgSrc+'" alt=""/></div>';
        str2+=i===0?'<li class="bg"></li>':'<li></li>';
        }
       //给str1在多拼接一个div索引为0的那一项
        str1+='<div><img1 realImg="'+data[0].imgSrc+'" alt=""/></div>';
        oBoxInner.innerHTML=str1;
        oUl.innerHTML=str2;
      //改变oBoxInner的宽度
        oBoxInner.style.width=aDiv.length*aDiv[0].offsetWidth+'px';

    }
/*
    bind();
    function bind(){
        var str1='';//根据数据多少进行字符串拼接div
        var str2='';//根据数据多少，拼接li
        for(var i=0;i<data.length;i++){
            str1+='<div><img1 realImg="'+data[i].imgSrc+'"/></div>';
            str2+=i===0?'<li class="bg"><>':'<li><>';
        }
        //给str1再多拼接一个div索引为0的那一项
        str1+='<div><img1 src="'+data[0].imgSrc+'"/></div>';
        oBoxInner.innerHTML=str1;
        oUl.innerHTML=str2;
        //改变oBoxInner的宽度：
        oBoxInner.style.width=aDiv.length*aDiv[0].offsetWidth+'px';
    }*/
// 4.延迟加载
    setTimeout(lazyImg,300)
  function lazyImg(){
      for(var i=0;i<aImg.length;i++){
     //思路1：自定义属性避免异步问题
     /* var tmpImg=new Image;
          tmpImg.src=aImg[i].getAttribute('realImg');
          tmpImg.index=i;
          //console.log(tmpImg.src)
          tmpImg.onload=function(){//  是异步，里面的i值一定会出问题
              aImg[this.index].src=this.src

          }*/
          //思路2.用闭包避免异步中的i值问题
          (function(index){
              if(aImg[index].loaded){
                  return
              }
              var tmpImg=new Image;
              tmpImg.src=aImg[index].getAttribute('realImg');
              tmpImg.onload=function(){
                  aImg[index].src=this.src;
                  tmpImg=null;
                  aImg[index].loaded=true
              };
              tmpImg.onerror=function(){
                  tmpImg=null;
              }
           })(i)
      }
  }
//    5.图片自动轮播
    clearInterval(autoTimer);
    autoTimer=setInterval(autoMove,interval);
    function autoMove(){
        if(step>=aDiv.length-1){
            step=0;
            utils.css(oBoxInner,'left',0)
        }
        step++;
        zhufengAnimate(oBoxInner,{left:-step*1000},500);
        //console.log(step)
        bannerTip()
    }
//    6.焦点自动轮播
    function bannerTip(){
        var tmpStep=step>=aLi.length?0:step;
        for(var i=0;i<aLi.length;i++){
            i===tmpStep?utils.addClass(aLi[i],'bg'):utils.removeClass(aLi[i],'bg')
        }
    }
    //7. 移入移出
    oBox.onmouseover=function(){
        clearInterval(autoTimer)
        aA[0].style.display=aA[1].style.display='block'

    }
    oBox.onmouseout=function(){
        autoTimer=setInterval(autoMove,1000);
        //utils.css(aA[0],'display','none')
        //utils.css(aA[1],'display','none')
        aA[0].style.display=aA[1].style.display='none'
    }

    // 8.点击焦点手动切换
    handleChange();
    function  handleChange(){
        for(var i=0;i<aLi.length;i++){
            aLi[i].index=i;
            aLi[i].onclick=function(){
                step=this.index;
                zhufengAnimate(oBoxInner,{left:-step*1000},500);
                bannerTip();
            }
        }
    }

    // 9.点击左右按钮切换
    aA[0].onclick=function(){
        if(step<=0){
            step=aLi.length;
            utils.css(oBoxInner,'left',-step*1000)
        }
        step--;
        zhufengAnimate(oBoxInner,{left:-step*1000},500)
        bannerTip()
    }
    aA[1].onclick=autoMove;

})()

























